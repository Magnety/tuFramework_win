...
class Deep_Attention_block(nn.Module):
    ...
    def __init__(self, F_up, F_down, F_out, conv_op=nn.Conv2d, conv_kwargs=None,
        ...
        self.fuse = nn.Sequential(
            ...
            self.conv_op(360, 3 * F_out, kernel_size=1), self.norm_op(3 * F_out, **self.norm_op_kwargs),
            self.nonlin(**self.nonlin_kwargs),
            self.conv_op(3 * F_out, 2 * F_out, kernel_size=3, padding=1),
            self.norm_op(2 * F_out, **self.norm_op_kwargs), self.nonlin(**self.nonlin_kwargs),
            self.conv_op(2 * F_out, F_out, kernel_size=3, padding=1), self.norm_op(F_out, **self.norm_op_kwargs),
            self.nonlin(**self.nonlin_kwargs),
        )
        ...
        self.out_att = nn.Sequential(
            self.conv_op(2 * F_out, F_out, kernel_size=1), self.norm_op(F_out, **self.norm_op_kwargs),
            self.nonlin(**self.nonlin_kwargs),
            self.conv_op(F_out, F_out, kernel_size=3, padding=1), self.norm_op(F_out, **self.norm_op_kwargs),
            self.nonlin(**self.nonlin_kwargs),
            self.conv_op(F_out, F_out, kernel_size=3, padding=1), self.norm_op(F_out, **self.norm_op_kwargs),
            self.nonlin(**self.nonlin_kwargs), )
...
class VNet_recons(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        channels = 12
        self.in_tr = InputTransition(input_channels=1, output_channels=channels, conv_op=self.conv_op,
        ...
        self.down_tr32 = DownTransition(input_features=channels, num_conv=1, first_stride=(1, 2, 2),
                                        conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                        norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                        dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                        nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                        basic_block=ConvDropoutNormNonlin, dropout=False)  # 32
        self.down_tr64 = DownTransition(input_features=2 * channels, num_conv=2, first_stride=(1, 2, 2),
                                        conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                        norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                        dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                        nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                        basic_block=ConvDropoutNormNonlin, dropout=False)  # 64
        self.down_tr128 = DownTransition(input_features=4 * channels, num_conv=3, first_stride=(1, 2, 2),
                                         conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                         norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                         dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                         nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                         basic_block=ConvDropoutNormNonlin, dropout=True)  # 128
        self.down_tr256 = DownTransition(input_features=8 * channels, num_conv=2, first_stride=1, conv_op=self.conv_op,
                                         conv_kwargs=self.conv_kwargs,
                                         norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                         dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                         nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                         basic_block=ConvDropoutNormNonlin, dropout=False)  # 256

        self.recons1 = ReconsTransition(input_features=2 * channels, output_features=channels, conv_op=self.conv_op,
                                        conv_kwargs=self.conv_kwargs,
                                        norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                        dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                        nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)
        self.recons2 = ReconsTransition(input_features=4 * channels, output_features=2 * channels, conv_op=self.conv_op,
                                        conv_kwargs=self.conv_kwargs,
                                        norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                        dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                        nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)

        self.att_block1 = Deep_Attention_block(F_up=16 * channels, F_down=8 * channels, F_out=8 * channels,
                                               conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                               norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                               dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                               nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)
        self.att_block2 = Deep_Attention_block(F_up=16 * channels, F_down=4 * channels, F_out=4 * channels,
                                               conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                               norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                               dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                               nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)
        self.att_block3 = Deep_Attention_block(F_up=8 * channels, F_down=2 * channels, F_out=2 * channels,
                                               conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                               norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                               dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                               nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)
        ...
        self.up_tr256 = UpTransition(input_features=16 * channels, output_features=16 * channels, num_conv=2,
                                     first_stride=1, conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                     norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                     dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                     nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                     basic_block=ConvDropoutNormNonlin, dropout=True)
        self.up_tr128 = UpTransition(input_features=16 * channels, output_features=8 * channels, num_conv=2,
                                     first_stride=2, conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                     norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                     dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                     nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                     basic_block=ConvDropoutNormNonlin, dropout=True)
        self.up_tr64 = UpTransition(input_features=8 * channels, output_features=4 * channels, num_conv=1,
                                    first_stride=2, conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                    norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                    dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                    nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                    basic_block=ConvDropoutNormNonlin)
        self.up_tr32 = UpTransition(input_features=4 * channels, output_features=2 * channels, num_conv=1,
                                    first_stride=2, conv_op=self.conv_op, conv_kwargs=self.conv_kwargs,
                                    norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                    dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                    nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs,
                                    basic_block=ConvDropoutNormNonlin, dropout=True)
        self.out_tr = OutputTransition(input_features=2 * channels, output_features=num_classes, conv_op=self.conv_op,
                                       conv_kwargs=self.conv_kwargs,
                                       norm_op=self.norm_op, norm_op_kwargs=self.norm_op_kwargs,
                                       dropout_op=self.dropout_op, dropout_op_kwargs=self.dropout_op_kwargs,
                                       nonlin=self.nonlin, nonlin_kwargs=self.nonlin_kwargs)
    ...
    def forward(self, x):
        ...
        # recons
        recons1_up = F.interpolate(down1, size=inconv.size()[2:], mode='trilinear',align_corners=True)
        recon1 = self.recons1(recons1_up)

        recons2_up = F.interpolate(down2, size=down1.size()[2:], mode='trilinear',align_corners=True)
        ...
        #
        up1 = F.interpolate(down4, size=down3.size()[2:], mode='trilinear',align_corners=True)
        # print('up1:', up1.shape)
        ...
        #
        up2 = F.interpolate(up1_conv, size=down2.size()[2:], mode='trilinear',align_corners=True)
        # print('up2:', up2.shape)
        ...
        #
        up3 = F.interpolate(up2_conv, size=down1.size()[2:], mode='trilinear',align_corners=True)
        # print('up3:', up3.shape)
        ...
        up4 = F.interpolate(up3_conv, size=inconv.size()[2:], mode='trilinear',align_corners=True)
        # print('up4:', up4.shape)
